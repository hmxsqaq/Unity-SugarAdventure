			...
			if(jellySprite.m_Style == JellySprite.PhysicsStyle.Free)
			{
				if(jellySprite.m_FreeModeBodyPositions != null)
				{
					...
					for(int i = 0; i < jellySprite.m_FreeModeBodyPositions.Count; i++)
					{
						...
						var fmh_554_91_638696459674322108 = Quaternion.identity; Vector3 newPosition = transform.InverseTransformPoint(Handles.FreeMoveHandle(point, 0.1f, Vector3.zero, Handles.DotHandleCap));
						newPosition.z = 0.0f;
