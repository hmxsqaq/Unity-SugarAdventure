...
namespace HighlightPlus2D {
	...
	public partial class HighlightEffect2D : MonoBehaviour {
		...
		void OnEnable() {
			...
			ignoreCachedSprite = GetComponentInChildren<Animator>(true);
			...
			for (int k = 0; k < rmsCount; k++) {
				...
				if (renderer is SpriteRenderer) {
					...
					if (polygonPacking) {
						...
						if (rms[k].quadMesh == null || rms[k].quadMesh == defaultQuadMesh) {
				...
				//Vector4 geom = new Vector4(rms[k].center.x, rms[k].center.y, rms[k].aspectRatio * viewportAspectRatio, 0);
				Vector4 geom = new Vector4(rms[k].center.x, rms[k].center.y, rms[k].aspectRatio, 0);
				if (glow > 0) {
			...
			if (_highlighted) {
				...
				if (glow > 0) {
					for (int k = 0; k < rms.Length; k++) {
						...
						Mesh quadMesh = rms[k].quadMesh;
						float baseGlowWidth = (1080f / Screen.width) * glowWidth / 100f;
						if (useGPUInstancing) {
							// GPU instancing glow
							propMatrices.Clear();
							propPivots.Clear();
							...
							for (int j = 0; j < glowPasses.Length; j++) {
								if (glowQuality != QualityLevel.Simple) {
									Vector4 glowData = new Vector4(glowReal * glowPasses[j].alpha, baseGlowWidth, glowMagicNumber1, glowMagicNumber2);
									...
									propGlowColor.Add(glowPasses[j].color);
									propGlowData.Add(new Vector4(glowReal * glowPasses[j].alpha, glowPasses[j].offset * baseGlowWidth, glowMagicNumber1, glowMagicNumber2));
									propMatrices.Add(matrix);
								}
							}
							...
							for (int j = 0; j < glowPasses.Length; j++) {
								...
								if (glowQuality != QualityLevel.Simple) {
									Vector2[] offsets = glowQuality == QualityLevel.High ? offsetsHQ : offsetsMQ;
									glowProps.SetVector(ShaderParams.Glow, new Vector4(glowReal * glowPasses[j].alpha, baseGlowWidth, glowMagicNumber1, glowMagicNumber2));
									float mult = glowPasses[j].offset * glowWidth;
									Vector2 disp = new Vector2(mult / rms[k].rectWidth, mult / rms[k].rectHeight);
									for (int z = 0; z < offsets.Length; z++) {
									...
									glowProps.SetVector(ShaderParams.Glow, new Vector4(glowReal * glowPasses[j].alpha, glowPasses[j].offset * baseGlowWidth, glowMagicNumber1, glowMagicNumber2));
									DrawSubMeshes(quadMesh, rms[k].renderer, matrix, rms[k].fxMatGlow, layer, glowProps);
								}
							}
						}
				...
				if (outline > 0) {
					...
					for (int k = 0; k < rms.Length; k++) {
						...
						if (outlineQuality != QualityLevel.Simple) {
							...
							rms[k].fxMatOutline.SetFloat(ShaderParams.OutlineWidth, (1080f / Screen.width) * disp.x);
		...
		Mesh SpriteToMesh(Sprite sprite) {
			if (ignoreCachedSprite || !cachedMeshes.TryGetValue(sprite, out Mesh mesh) || mesh == null) {
				mesh = new Mesh();
